...
Due to the file size limitations, we were unable to upload the raw data to the Git repository. Nonetheless, we have uploaded the processed data files that were used for further analysis. Details of the wrangling process from raw to processed data can be referenced from the "Code" folder in the Git repository. 
...
visualizeVariables <- function(selected_state, selected_pollutant){
  ...
  return(list(df_energy.air.data, plot_energy.air))}
    ...
    plot_energy.air <- ggplot(df_energy.air.data, aes(x = NetGeneration, y = MeanNO2)) +
      ...
      geom_smooth(method = "lm", color = "red") +
      labs(x="Net Generation (TWh)",
           y="Nitrogen Dioxide Concentration (ppb)",
           title= paste0("Nitrogen Dioxide Concentration (ppb) by Net Generation (TWh)
                         in ",
                         df_state$State[df_state$Abbreviation == selected_state]))
    return(list(df_energy.air.data, plot_energy.air))
